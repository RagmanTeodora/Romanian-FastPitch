...
export FREEZE_PRED_ALL="False"
export FREEZE_PITCH="False"
export FREEZE_DUR="True"
export FREEZE_ENG="False"

string=""

if [ "$FREEZE_ENC" == "True" ]; then
    string="${string}_frenc"
fi

if [ "$FREEZE_DEC" == "True" ]; then
    string="${string}_frdec"
fi

if [ "$FREEZE_PRED_ALL" == "True" ]; then
    string="${string}_frpred"
fi

if [ "$FREEZE_PITCH" == "True" ]; then
    string="${string}_frpitch"
    export FREEZE_PRED_ALL="False"
fi

if [ "$FREEZE_DUR" == "True" ]; then
    string="${string}_frdur"
    export FREEZE_PRED_ALL="False"
fi

if [ "$FREEZE_ENG" == "True" ]; then
    string="${string}_freng"
    export FREEZE_PRED_ALL="False"
fi
...
for FASTPITCH_config in "${configs[@]}"; do
    ...
    OUTPUT_DIR=./OUTPUT_MODELS/frozen/${SPEAKER}${string}_${FASTPITCH_config}/
